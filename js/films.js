	...
	$.ajax({
		...
		success: function (data) {
			if (data.moviePlotSimple.length > 130) {
				newmoviePlotSimple = data.moviePlotSimple.substring(0, 127) + "...<span style='color: #6398d7'>(查看更多)</span>";
			} else {
				newmoviePlotSimple = data.moviePlotSimple;
			}
			allMoviePlotSimple = data.moviePlotSimple;
			var tbhtml = "<header><div class='profilePhoto'><img src= '" + data.moviePoster + "' alt='sample'></div><section class='profileHeader'><h2 style='color: #000;'>" + data.movieTitle + "</h2><h3>" + data.movieAlsoKnownAs + "</h3><hr><p id='demo'>" + newmoviePlotSimple + "</p></section></header><hr><div class='detail-more'><p class='newP'><span>创作者 :</span>" + data.movieWriters + "</p><p class='newP'><span>导演 :</span>" + data.movieDirectors + "</p><p class='newP'><span>主演 :</span>" + data.movieActors + "</p><p class='newP'><span>地区 :</span>" + data.movieCountry + "</p><p class='newP'><span>时长 :</span>" + data.movieRuntime + "</p><p class='newP'><span>类型 :</span>" + data.movieGenres + "</p><p class='newP'><span>评分 :</span>" + data.movieRating + "</p></div><hr>";
			$("#details").html(tbhtml);
		},
		error: function () {
		}
